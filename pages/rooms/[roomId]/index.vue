...
const { setRoomInfo, setBookingInfo } = bookingStore;
...
const takeReservation = () => {
  ...
  const bookingDays = countDateDiffs({
    start: bookingDate.value.start,
    end: bookingDate.value.end,
  });
  const checkInDate = bookingDate.value.start.replaceAll('-', '/');
  const checkOutDate = bookingDate.value.end.replaceAll('-', '/');

  setBookingInfo({ bookingDays, checkInDate, checkOutDate, bookingPeople });
